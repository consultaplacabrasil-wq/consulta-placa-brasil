...
export function FipeVariation({ codigo }: { codigo: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/fipe/${encodeURIComponent(codigo)}`)
      .then((r) => r.json())
      .then((d) => setPrecos(Array.isArray(d.precos) ? d.precos : []))
      .catch(() => setPrecos([]))
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 0", color: "#94a3b8" }}>
        ...
        <span style={{ fontSize: 13 }}>Consultando tabela FIPE...</span>
      </div>
    );
  }
  ...
  // Usa até os últimos 6 registros
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 8, height: 130, paddingTop: 8 }}>
        {serie.map((s, i) => {
          const h = 20 + ((s.valor - min) / range) * 80; // 20%..100%
